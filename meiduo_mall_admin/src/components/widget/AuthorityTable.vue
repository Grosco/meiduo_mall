    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="fnPopShow(scope.row.id,scope.row.name,scope.row.codename)">编辑
          </el-button>
          <el-button
            size="mini"
            type="danger"
            @click="fnDelAuthor(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="编辑权限" :visible.sync="pop_show" append-to-body>
      <el-form :model="authorForm" status-icon :rules="rulesAuthorForm" ref="authorForm" label-width="100px">
        <el-form-item label="权限名称：" prop="name">
          <el-input type="text" v-model="authorForm.name" autocomplete="off" size="small"
                    class="input_width"></el-input>
        </el-form-item>
        <el-form-item label="权限识别名：" prop="codename">
          <el-input type="text" v-model="authorForm.codename" autocomplete="off" size="small"
                    class="input_width"></el-input>
        </el-form-item>
